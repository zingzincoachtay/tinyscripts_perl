...
my $param = {'rep'=>int rand(1024)
            ,'set'=>10
            ,'job'=>['while']
            ,'lap'=>{ 'for'=>[],'foreach'=>[],'foranon'=>[],'do'=>[],'while'=>[],'whileeach'=>[],'switch'=>[],'if'=>[] };
};
...
sub strenuous {
  ...
  SWITCH: for(@$job) {
        'for' && foreach(1 .. $set) { push(@{$lap->    {'for'}},    &test_for(0,shift @_) )}
    'foreach' && foreach(1 .. $set) { push(@{$lap->{'foreach'}},&test_foreach(1,shift @_) )}
    'foranon' && foreach(1 .. $set) { push(@{$lap->{'foranon'}},&test_foranon(1,shift @_) )}
           'do' && foreach(1 .. $set) { push(@{$lap->       {'do'}},       &test_do(1,shift @_) )}
        'while' && foreach(1 .. $set) { push(@{$lap->    {'while'}},    &test_while(0,shift @_) )}
    'whileeach' && foreach(1 .. $set) { push(@{$lap->{'whileeach'}},&test_whileeach(1,shift @_) )}
    /^length$/i && do{};
    # array size, `length` function vs array indexing
    /^strpos$/i && do{};
